...
class MysqlHelper():
    ...
    conn_paramsl={'host':"localhost",'port':3306,'user':"root",'passwd':"root",'db':"mytestdb",'charset':'utf8'}
    ...
    # 建立数据库连接和打开游标
    def __connect(self):
        self.__conn=connect(host=self.__host,port=self.__port,db=self.__db,user=self.__user,passwd=self.__passwd,charset=self.__charset)
    ...
    # 写数据操作具体实现，增删改操作都是调用这个方法来实现，这个是室友方法，不允许类外部调用
