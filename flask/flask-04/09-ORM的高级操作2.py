...
#不用把数据全部从关系数据库中读出来，但是可以进行追加和过滤等操作
def test_lazy():
    d=session.query(Detp).filter(Detp.dept_no==1)
    print(type(d))
    result=d.first()
    e=Emp(e_name="xxx",job="cherk",hire_date=date(2022,5,26),sal=2222)
    result.emps.append(e)
    re=result.emps.filter(Emp.emp_no>2).all()
    print(re[0])
