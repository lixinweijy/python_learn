...
class Hclient(wx.Frame):
    def __init__(self,c_name):
        ...
        #创建两个按钮
        conn_buntton=wx.Button(p1,size=(200,40),label="连接")
        dis_conn_buntton=wx.Button(p1,size=(200,40),label="离开")

        g1.Add(conn_buntton,1,wx.TOP|wx.LEFT)  #连接按钮布局在左边
        g1.Add(dis_conn_buntton, 1, wx.TOP | wx.RIGHT)  #断开按钮布局在右边

        box.Add(g1,1,wx.ALIGN_CENTER) #ALIGN_CENTER联合居中

        #创建聊天内容的文本框，不能写消息
        self.read_text=wx.TextCtrl(p1,size=(400,250),style=wx.TE_MULTILINE | wx.TE_READONLY)
        ...
        clear_buntton = wx.Button(p1, size=(200, 40), label="重置")
        send_buntton = wx.Button(p1, size=(200, 40), label="发送")
        g2.Add(clear_buntton,1,wx.TOP | wx.LEFT)
        g2.Add(send_buntton,1,wx.TOP | wx.RIGHT)
        ...
        """给所有按钮绑定点击事件"""
        self.Bind(wx.EVT_BUTTON,self.connect_to_server,conn_buntton)
        self.Bind(wx.EVT_BUTTON,self.send_to,send_buntton)
        self.Bind(wx.EVT_BUTTON,self.go_out,dis_conn_buntton)
        self.Bind(wx.EVT_BUTTON,self.reset,clear_buntton)
        """客户端的属性"""
        self.name=c_name
